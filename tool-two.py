...
def plot_all_graphs(protocol_counts, ip_communication_protocols):
    ...
    fig, ax = plt.subplots()
    #print(protocol_counts.values.tolist())
    #print("")
    #print(ip_communication_protocols.values.tolist())
    #ax.bar(ip_communication_protocols.values.tolist(), protocol_counts.values.tolist(), label=bar_labels, color=bar_colors)
    items = []
    counts = []
    ...
    fig.savefig('/var/www/html/files/my_plot.png')
    items.clear()
    counts.clear()
    fig2, ax2 = plt.subplots(figsize=(6, 5))
    for i in range(len(ip_communication_protocols.values.tolist())):
        items.append(ip_communication_protocols.values.tolist()[i][0]+ " to " +ip_communication_protocols.values.tolist()[i][1])
    ...
    fig2.savefig('/var/www/html/files/my_plot2.png', bbox_inches='tight')
